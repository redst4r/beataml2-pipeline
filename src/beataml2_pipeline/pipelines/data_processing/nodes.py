...
def load_mapping(df_sample_mapping: pd.DataFrame):
    ...
    df_sample_mapping["dbgap_subject_id"] = _subject_id_to_patient(
        df_sample_mapping["dbgap_subject_id"]
    )
    return df_sample_mapping


def _consolidate_dna_rna_id_to_labId(
    rna_series: pd.Series, dna_series: pd.Series, df_mapping: pd.DataFrame
):
    ...
    """
    rna_to_lab = (
        df_mapping.dropna(subset=["dbgap_rnaseq_sample", "labId"])
        .set_index("dbgap_rnaseq_sample")
        .labId.to_dict()
    )
    dna_to_lab = (
        df_mapping.dropna(subset=["dbgap_dnaseq_sample", "labId"])
        .set_index("dbgap_dnaseq_sample")
        .labId.to_dict()
    )
...
def load_clinical(df_clinical: pd.DataFrame, df_mapping: pd.DataFrame):
    ...
    df_clinical["labId"] = _consolidate_dna_rna_id_to_labId(
        df_clinical.dbgap_rnaseq_sample, df_clinical.dbgap_dnaseq_sample, df_mapping
    )

    # fix the data
    df_clinical["%.Blasts.in.PB"] = (
        df_clinical["%.Blasts.in.PB"]
        .replace(
            {
                ">90": "90",
    ...
    df_clinical["%.Blasts.in.BM"] = (
        df_clinical["%.Blasts.in.BM"]
        .replace({">90": "90", ">95": "95", "N/A ": np.nan, "<1": 1})
        .astype(float)
    )

    df_clinical["cumulativeTreatmentTypes"] = df_clinical[
        "cumulativeTreatmentTypes"
    ].fillna("unknown")
    df_clinical["had_bm_transplant"] = df_clinical["cumulativeTreatmentTypes"].apply(
        lambda x: "y" if "Bone Marrow Transplant" in x else "n"
    )
    df_clinical["cumulativeTreatmentStages"] = df_clinical[
        "cumulativeTreatmentStages"
    ].fillna("unknown")
    df_clinical["had_bm_transplant2"] = df_clinical["cumulativeTreatmentStages"].apply(
        lambda x: "y" if "Allogeneic" in x else "n"
    )
...
def load_drugs(df_drug, df_family_dict, df_map):
    ...
    df_drug = df_drug.merge(
        df_drug_family[["inhibitor", "family"]], on="inhibitor", how="left"
    )
    df_drug["family"] = df_drug["family"].fillna("unknown")
    df_drug["dbgap_subject_id"] = _subject_id_to_patient(df_drug["dbgap_subject_id"])

    df_drug["labId"] = _consolidate_dna_rna_id_to_labId(
        df_drug.dbgap_rnaseq_sample, df_drug.dbgap_dnaseq_sample, df_map
    )
    df_drug = df_drug.drop(["dbgap_dnaseq_sample", "dbgap_rnaseq_sample"], axis=1)

    df_ic50 = pd.crosstab(
        df_drug.labId, df_drug.inhibitor, values=df_drug.ic50, aggfunc="mean"
    )
    adata_drug_IC50 = anndata.AnnData(df_ic50)

    df_auc = pd.crosstab(
        df_drug.labId, df_drug.inhibitor, values=df_drug.auc, aggfunc="mean"
    )
...
def load_mutations(df_mut, df_mut_suppl, df_mapping, df_clinical_intermediate):
    ...
    # add patient info
    dna_to_patient = df_mapping.set_index(
        "dbgap_dnaseq_sample"
    ).dbgap_subject_id.to_dict()
    ...
    flt3_itd_status = (
        df_clinical_intermediate.set_index("labId")["FLT3-ITD"]
        .replace({"positive": 1.0, "negative": 0.0, "unknown": np.nan})
        .to_dict()
    )
    df_mut_gene["FLT3-ITD"] = df_mut_gene.index.map(
        lambda x: flt3_itd_status[x] if x in flt3_itd_status else np.nan
    )
